/**
 * api.ts - THE MASTERMIND (V5.3 - ZERO TRUST ARCHITECTURE)
 * ────────────────────────────────────────────────────────
 * Features: Safe JSON Parsing, Direct-Link Handshake, Silent Token Rotation
 */
...
const valveFetch = async (url: string, signal?: AbortSignal, isRetry = false): Promise<any> => {
    try {
        ...
        // 🛡️ SAFE PARSING: Prevent HTML crash
...
const triggerAction = async (url: string, method: 'POST' | 'PATCH' | 'PUT' | 'DELETE', body?: any, isRetry = false): Promise<any> => {
    try {
        ...
        // 🛡️ SAFE PARSING: Prevent HTML crash on 404s
        const contentType = response.headers.get("content-type");
        let data;
        if (contentType && contentType.includes("application/json")) {
...
export const ApiService = {
    ...
    saveResident: (id: string | undefined, payload: any) => triggerAction(id ? `${RESIDENTS_API}/${id}` : RESIDENTS_API, id ? 'PUT' : 'POST', payload),
    ...
    saveHousehold: (id: string | undefined, payload: any) => triggerAction(id ? `${HOUSEHOLDS_API}/${id}` : HOUSEHOLDS_API, id ? 'PUT' : 'POST', payload),
    ...
    saveOfficial: (id: string | undefined, payload: any) => triggerAction(id ? `${OFFICIALS_API}/${id}` : OFFICIALS_API, id ? 'PUT' : 'POST', payload),
    ...
    saveAnnouncement: (id: string | null, payload: any) => triggerAction(id ? `${ANNOUNCEMENT_API}/${id}` : ANNOUNCEMENT_API, id ? 'PUT' : 'POST', payload),
    ...
    saveDocumentRecord: (payload: any) => triggerAction(payload.id ? `${DOCUMENTS_API}/${payload.id}` : `${DOCUMENTS_API}/save`, payload.id ? 'PUT' : 'POST', payload),
    ...
    saveBlotter: (id: string | null, payload: any) => triggerAction(id ? `${BLOTTER_API}/${id}` : BLOTTER_API, id ? 'PUT' : 'POST', payload),
